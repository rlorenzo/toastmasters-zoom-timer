...
The MediaPipe bundle, its WASM loader, and the segmenter model are all pinned to an exact version/path in `app.js` (no `latest` or floating range); browser support for CSP path scoping varies, so these exact-version URLs are the primary control, and `index.html`'s CSP mirrors the same versioned jsDelivr path across `script-src`, `connect-src`, `worker-src`, and `child-src` as defense in depth. To bump the MediaPipe version: pick a new exact `@mediapipe/tasks-vision` release (`npm view @mediapipe/tasks-vision versions`), update the `MEDIAPIPE_VERSION` constant in `app.js` (the `vision_bundle.mjs` and `wasm` URLs are derived from it), update the matching `@<version>/` string in all four of `index.html`'s CSP entries above so the pinned bundle and its WASM worker aren't blocked, and re-check the model bucket listing for a matching numbered path if one is published.
